...
contract STAKDBounty is Owned {

  mapping (address => bool) public users;

  mapping (address => uint256) public social;

  uint256 public stakdPerUser;
  ...
  function STAKDBounty(address[] owners, address _token, uint256 _stakdPerUser) public {
    require(_token != address(0x0));
    require(_stakdPerUser > 0);
    setOwners(owners);
    token = Token(_token);
    stakdPerUser = _stakdPerUser;
  }
  ...
  function balanceOf(address user) public constant returns (uint256) {
    uint256 stakds = social[user];
    if (users[user]) stakds += stakdPerUser;
    return stakds;
  }
